...
# 데이터 로드
results_df <- readRDS("./Korea_pub/final_merged_resultsK.RDS")
...
print("Available effect size levels:")
...
#### 생성 데이터 효과도 저장한 결과 ####
results_df2 <- readRDS("./Korea_pub/final_merged_resultsK_d.RDS")
results_df2 %>% select(BF_jzs, BF_gica, std_mean_diff, sdr, delta, scenario) -> results_df3
# 데이터 변환 및 시각화를 위한 코드
result_with_d <- results_df2 %>%
  # BF와 표준화된 평균차를 사용한 분석
  pivot_longer(
    cols = starts_with("BF_"),
    names_to = "model",
    values_to = "BF"
...
# 특이한 패턴이 있는지 scatter plot으로도 확인
ggplot(result_ratio, aes(x = std_mean_diff, y = bf_ratio)) +
  geom_point(alpha = 0.1) +
  facet_wrap(~scenario)

# BF_scatter plot with alpha
ggplot(result_with_d, aes(x = std_mean_diff, y = BF)) +
